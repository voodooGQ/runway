...
class Cleanup(Blueprint):
    ...
    def _get_replicated_lambda_remover_lambda(self):
        ...
        res['function'] = self.template.add_resource(
            awslambda.Function(
                'ReplicatedLambdaRemover',
                Code=awslambda.Code(
                    ZipFile=self._get_code_template(
    ...
    def _get_self_destruct(self, replicated_lambda_remover):
        ...
        res['role'] = self.template.add_resource(
            iam.Role(
                ...
                Policies=[
                    ...
                    iam.Policy(
                        PolicyName="DeleteRolePolicies",
                        PolicyDocument=PolicyDocument(
                            Version="2012-10-17",
                            Statement=[
                                Statement(
                                    Action=[awacs.iam.DeleteRolePolicy],
                                    Effect=Allow,
                                    # TODO: This is not working correctly unless keeping it all
                                    # open
                                    Resource=['*']
                                )
                            ]
                                ...
                                Statement(
                                    ...
                                    Resource=[
                                        ...
                                        Join('', [
                                            ...
                                            ':iam::',
                                            AccountId,
                                            ':role/%s-StateMachineRole-*' % (
                                                variables['stack_name']
                                            ),
                                        ]),
                                        replicated_lambda_remover['role'].get_att('Arn'),
                                    ]
                                )
                            ]
                        )
        ...
        res['function'] = self.template.add_resource(
            awslambda.Function(
                'SelfDestruct',
                Code=awslambda.Code(
                    ZipFile=self._get_code_template('templates/self_destruct.template.py')
                ),
    ...
    def _get_replicated_lambda_state_machine_role(self,
                                                  remover_function,  # noqa pylint: disable=line-too-long # type: Dict[str, Union(awslambda.Function, iam.Role, Any)]
                                                  self_destruct_function,  # noqa pylint:disable=line-too-long # type: Dict[str, Union(awslambda.Function, iam.Role, Any)]
                                                 ):
        # type (...) -> iam.Role
